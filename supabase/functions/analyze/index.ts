...
const SYSTEM_PROMPT = `Tu es un expert philatéliste et numismate de renommée mondiale avec 40 ans d'expérience. Tu as expertisé des collections pour Christie's, Sotheby's, Heritage Auctions. Tu utilises les catalogues Stanley Gibbons, Scott, Yvert & Tellier (philatélie) et NGC, PCGS, Krause World Coins (numismatique).
...
function normalizeImageUrlForRuntime(imageUrl: string): string {
  ...
  try {
    ...
    // En local, le runtime Edge tourne dans un conteneur Docker.
    // Les URLs signees generees cote client pointent vers 127.0.0.1:54321,
    // qui n'est pas resolu vers la gateway Supabase depuis ce conteneur.
    if (
...
// Télécharge une image et la convertit en base64 (gère aussi les data: URIs)
async function fetchImageAsBase64(imageUrl: string): Promise<{ base64: string; mediaType: string }> {
  const runtimeImageUrl = normalizeImageUrlForRuntime(imageUrl);

  // data: URI — extraction directe sans téléchargement
  if (runtimeImageUrl.startsWith("data:")) {
    const comma = runtimeImageUrl.indexOf(",");
    if (comma === -1) throw new Error("INVALID_DATA_URI");
    const header = runtimeImageUrl.slice(5, comma); // ex: "image/jpeg;base64"
    const base64 = runtimeImageUrl.slice(comma + 1);
    const mediaType = header.replace(";base64", "") || "image/jpeg";
    return { base64, mediaType };
  }

  // URL HTTP/HTTPS — téléchargement
  const res = await fetch(runtimeImageUrl, {
    headers: { "User-Agent": "CollectScan/2.0 (educational)" },
  });
...
serve(async (req) => {
  ...
  try {
    ...
    if (!imageUrl || !type) {
      return new Response(JSON.stringify({ error: "imageUrl and type required" }), {
        status: 400,
    ...
    // Télécharger l'image en base64 pour éviter les restrictions d'URL côté Claude
    let imageSource: Record<string, unknown>;
    try {
      const { base64, mediaType } = await fetchImageAsBase64(imageUrl);
      imageSource = { type: "base64", media_type: mediaType, data: base64 };
    } catch (e) {
      // Fallback : envoyer l'URL directement si téléchargement impossible
    ...
    if (!response.ok) {
      ...
      throw new Error(`CLAUDE_API_ERROR_${response.status}: ${errText.slice(0, 200)}`);
    }
    ...
    if (jsonStart === -1 || jsonEnd <= jsonStart) throw new Error("AI_INVALID_JSON");
